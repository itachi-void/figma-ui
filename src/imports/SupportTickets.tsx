...
import { useTickets, Ticket, TicketStatus, TicketPriority } from '@/app/contexts/TicketsContext';
import { useRole } from '@/app/contexts/RoleContext';

export default function SupportTickets() {
  const { role } = useRole();
  const { tickets, updateTicketStatus, createTicket } = useTickets();
  const [searchQuery, setSearchQuery] = useState('');
  const [filterStatus, setFilterStatus] = useState<TicketStatus | 'all'>('all');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [newTicket, setNewTicket] = useState({ subject: '', description: '', priority: 'low' as TicketPriority });

  // Mock current user names based on role (for demo purposes)
  const currentUserName = role === 'citizen' ? 'John Doe' : role === 'driver' ? 'Ahmed Hassan' : 'Admin';

  const filteredTickets = tickets.filter(t => {
    // Role-based visibility
    if (role === 'citizen' && t.citizenName !== currentUserName) return false;
    if (role === 'driver' && t.citizenName !== currentUserName && t.driverName !== currentUserName) return false;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createTicket({
      ...
      citizenName: role === 'citizen' ? currentUserName : (role === 'driver' ? 'System' : 'Admin'),
      driverName: role === 'driver' ? currentUserName : undefined,
    });
      ...
      >
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {role === 'citizen' ? 'Track your inquiries and complaints' : role === 'driver' ? 'Report vehicle issues or route problems' : 'Manage citizen complaints, inquiries, and driver ratings'}
          </p>
        </div>
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  <span className={`px-2.5 py-1 rounded-md text-xs font-bold border ${getStatusColor(ticket.status)}`}>
                ...
                <div className="flex items-center gap-4 text-sm font-medium text-gray-500">
                  <p>From: <span className="text-gray-900">{ticket.citizenName}</span></p>
                  {ticket.driverName && (
                    <p>Driver: <span className="text-gray-900">{ticket.driverName}</span></p>
                  )}
                </div>
              </div>
