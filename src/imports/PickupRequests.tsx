...
const mockRequests = [
  { id: 'REQ-101', citizen: 'John Doe', location: '123 Nile St, Downtown', bottles: 45, priority: 'High', status: 'Pending', time: '5m ago', lat: 30.0444, lng: 31.2357 },
  { id: 'REQ-102', citizen: 'Sarah J.', location: '45 Garden City', bottles: 120, priority: 'Critical', status: 'Pending', time: '12m ago', lat: 30.0333, lng: 31.2333 },
  { id: 'REQ-103', citizen: 'Mike Tyson', location: 'Industrial Zone B', bottles: 300, priority: 'High', status: 'Assigned', driver: 'Ahmed K.', time: '20m ago', lat: 30.0500, lng: 31.2000 },
  { id: 'REQ-104', citizen: 'Ali Ahmed', location: 'Maadi Hub 5', bottles: 60, priority: 'Normal', status: 'Pending', time: '45m ago', lat: 29.9667, lng: 31.2500 },
];

const availableDrivers = [
  { id: 'D-01', name: 'Ahmed Kamal', vehicle: 'Small Truck', status: 'On Route', load: 65, fuel: 40, lat: 30.0400, lng: 31.2400 },
  { id: 'D-02', name: 'Sarah Wilson', vehicle: 'Motorcycle', status: 'Available', load: 0, fuel: 85, lat: 30.0450, lng: 31.2300 },
  { id: 'D-03', name: 'Zainab Ali', vehicle: 'TukTuk', status: 'Available', load: 12, fuel: 15, lat: 30.0350, lng: 31.2500 }, // Low fuel
  { id: 'D-04', name: 'Omar Sayed', vehicle: 'Large Truck', status: 'Available', load: 90, fuel: 60, lat: 30.0420, lng: 31.2380 }, // High load
  { id: 'D-05', name: 'Fatima N.', vehicle: 'Van', status: 'Available', load: 30, fuel: 75, lat: 30.0480, lng: 31.2370 },
];
...
export default function PickupRequests() {
  ...
  const toggleAll = () => {
    const visibleIds = visibleRequests.filter(r => r.status.toLowerCase() === activeTab).map(r => r.id);
  ...
  const handleAutoAssign = () => {
    ...
    if (eligibleDrivers.length === 0) {
      notify.warning('Assignment Failed', 'No drivers available meeting the fuel (>20%) and capacity (<80%) requirements.');
      return;
    }

    // Find closest eligible driver
    let closestDriver = eligibleDrivers[0];
    let minDistance = calculateDistance(selectedReq.lat, selectedReq.lng, closestDriver.lat, closestDriver.lng);

    for (let i = 1; i < eligibleDrivers.length; i++) {
        const driver = eligibleDrivers[i];
        const dist = calculateDistance(selectedReq.lat, selectedReq.lng, driver.lat, driver.lng);
        if (dist < minDistance) {
            minDistance = dist;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[50vh] text-center space-y-4">
        ...
        <p className="text-gray-500 max-w-md">
          Pickup requests management is not available for Drivers. Please refer to your personalized Routes Management dashboard.
        </p>
      </div>
    );
  }
  ...
  let visibleRequests = role === 'citizen' 
    ? mockRequests.filter(r => r.citizen === 'John Doe')
    : mockRequests;

  if (filterPriority !== 'All') {
     visibleRequests = visibleRequests.filter(r => r.priority === filterPriority);
  }
  
  // Basic mock zone filtering logic (string matching)
  if (filterZone !== 'All') {
     visibleRequests = visibleRequests.filter(r => r.location.includes(filterZone));
  }
            ...
            <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-xl border border-gray-50 dark:border-gray-800 overflow-hidden">
              <div className="p-8 border-b border-gray-50 dark:border-gray-800 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                    <button onClick={() => setViewMode('list')} className={`p-1.5 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white dark:bg-gray-700 shadow-sm' : 'text-gray-500'}`}>
                      <Filter className="w-4 h-4" />
                    </button>
                    <button onClick={() => setViewMode('map')} className={`p-1.5 rounded-md transition-colors ${viewMode === 'map' ? 'bg-white dark:bg-gray-700 shadow-sm' : 'text-gray-500'}`}>
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Search requests..." className="pl-9 pr-4 py-2 bg-gray-50 dark:bg-gray-800 rounded-xl text-[10px] font-bold outline-none w-full md:w-[200px]" />
                  </div>
                </div>
              </div>
              ...
              {viewMode === 'map' ? (
                <div className="h-[500px] w-full bg-gray-100 dark:bg-gray-800 relative overflow-hidden rounded-b-[2.5rem] z-0">
                  <MapContainer center={[30.0444, 31.2357]} zoom={11} style={{ height: '100%', width: '100%', zIndex: 1 }}>
                    <TileLayer
                    ...
                    {visibleRequests.map((req) => (
                      <Marker key={req.id} position={[req.lat, req.lng]}>
                        <Popup>
                          <strong>{req.citizen}</strong><br/>
                          {req.bottles} Bottles<br/>
                          {req.location}
                        </Popup>
                      </Marker>
                    ))}
                ...
                <>
                  {role !== 'citizen' && visibleRequests.filter(r => r.status.toLowerCase() === activeTab).length > 0 && (
                    <div className="px-6 py-3 bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                      <label className="flex items-center gap-2 cursor-pointer group">
                        <div className="relative">
                          <input type="checkbox" onChange={toggleAll} checked={selectedRequests.length > 0 && selectedRequests.length === visibleRequests.filter(r => r.status.toLowerCase() === activeTab).length} className="peer sr-only" />
                        ...
                        <span className="text-[10px] font-bold text-gray-500 uppercase">Select All</span>
                      </label>
                      {selectedRequests.length > 0 && (
                        <div className="text-[10px] font-bold text-rose-600 bg-rose-50 px-2 py-1 rounded-md">{selectedRequests.length} Selected</div>
                      )}
                    </div>
                  )}
                  <div className="divide-y divide-gray-50 dark:divide-gray-800">
                    {visibleRequests.filter(r => r.status.toLowerCase() === activeTab).length > 0 ? (
                      visibleRequests.filter(r => r.status.toLowerCase() === activeTab).map((req) => (
                        <motion.div
                          key={req.id}
                          whileHover={{ x: 5 }}
                          onClick={() => role !== 'citizen' && setSelectedReq(req)}
                          className={`p-6 flex items-center justify-between group transition-all ${
                            role !== 'citizen' ? 'cursor-pointer hover:bg-gray-50/50' : ''
                          } ${selectedReq?.id === req.id || selectedRequests.includes(req.id) ? 'bg-rose-50/50 dark:bg-rose-900/10' : ''}`}
                        >
                          <div className="flex items-center gap-6">
                            {role !== 'citizen' && (
                              <div onClick={(e) => toggleSelection(req.id, e)} className="relative cursor-pointer flex-shrink-0">
                                <div className={`w-4 h-4 border-2 rounded flex items-center justify-center transition-colors ${selectedRequests.includes(req.id) ? 'bg-rose-500 border-rose-500 text-white' : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800'}`}>
                                  {selectedRequests.includes(req.id) && <CircleCheck className="w-3 h-3" />}
                                </div>
                              </div>
                            )}
                        ...
                        {role === 'citizen' ? (
                          <>
                            ...
                            <p className="text-gray-500 dark:text-gray-400 max-w-xs mx-auto mb-6">
                              You haven't made any pickup requests in this status. Start recycling today!
                            </p>
                          ...
                          <>
                            ...
                            <p className="text-gray-500 font-medium">No requests match this filters.</p>
                          </>
                        )}
                      </div>
                    )}
                  ...
                  <div className="p-6 bg-gray-50 dark:bg-gray-800 rounded-3xl mb-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="h-32 w-full bg-gray-200 dark:bg-gray-700 rounded-2xl flex items-center justify-center relative overflow-hidden shadow-inner z-0">
                         <MapContainer center={[selectedReq.lat, selectedReq.lng]} zoom={14} style={{ height: '100%', width: '100%', zIndex: 1 }} zoomControl={false} dragging={false}>
                            <TileLayer
                              url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                            />
                            <Marker position={[selectedReq.lat, selectedReq.lng]} />
                            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/50 to-transparent p-2 z-[400] pointer-events-none">
                                <span className="text-[10px] font-bold text-white uppercase tracking-wider">{selectedReq.location}</span>
                            </div>
                         </MapContainer>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-xs font-bold text-gray-900 dark:text-white">{selectedReq.bottles} Bottles</p>
                          <p className="text-[10px] text-gray-400">Recyclable Volume</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-4">
                         <div className="h-16 bg-gray-200 dark:bg-gray-700 rounded-xl flex items-center justify-center overflow-hidden">
                             <img src="https://images.unsplash.com/photo-1604187351574-c75ca79f5807?ixlib=rb-4.0.3&auto=format&fit=crop&w=150&q=80" alt="Recyclables" className="w-full h-full object-cover opacity-80" />
                         </div>
                         <div className="h-16 bg-gray-200 dark:bg-gray-700 rounded-xl flex items-center justify-center overflow-hidden">
                             <img src="https://images.unsplash.com/photo-1532996122724-e3c354a0b15b?ixlib=rb-4.0.3&auto=format&fit=crop&w=150&q=80" alt="Bags" className="w-full h-full object-cover opacity-80" />
                         </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-gray-400">
                              {driver.vehicle} • {driver.load}% Load • {driver.fuel}% Fuel
                              {selectedReq && ` • ${calculateDistance(selectedReq.lat, selectedReq.lng, driver.lat, driver.lng).toFixed(1)}km away`}
                            </p>
                          </div>
                        </div>
