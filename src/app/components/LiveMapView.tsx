...
const cairoLocations: Record<string, [number, number]> = {
  ...
  "Kasr El Nil Bridge": [30.043, 31.23],
  "Ramsis Square": [30.0622, 31.2469],
  "Cairo Tower": [30.0458, 31.2243],
  "Zamalek Club": [30.0561, 31.2194],
  "26th July Street": [30.057, 31.2175],
  "Brazil Street": [30.058, 31.215],
  "Heliopolis Square (El-Nozha)": [30.0961, 31.3306],
  "Nozha Street": [30.0975, 31.322],
  "Al-Ahram Street": [30.083, 31.315],
  "Merghany Street": [30.092, 31.328],
  "Abbas El-Akkad Street": [30.062, 31.335],
  "Makram Ebeid Street": [30.072, 31.328],
  "Nasr City Police Station": [30.058, 31.34],
  "El-Tayaran Street": [30.065, 31.332],
  "Road 9": [29.965, 31.262],
  "Road 233": [29.968, 31.26],
  "Corniche El-Maadi": [29.96, 31.26],
  "Maadi Grand Mall": [29.966, 31.265],
  "Giza Square": [30.0131, 31.2089],
  "Pyramids Road (Harram)": [30.025, 31.195],
  "Faisal Street": [30.022, 31.2],
  "Cairo University": [30.027, 31.21],
  "Dokki Square": [30.04, 31.215],
  Mohanadiseen: [30.053, 31.2],
  "Lebanon Square": [30.055, 31.205],
  "El-Batal Ahmed Abdel Aziz": [30.048, 31.21],
  "American University": [30.019, 31.5],
  "Fifth Settlement": [30.03, 31.47],
  "90th Street": [30.022, 31.48],
  "Shubra Street": [30.0833, 31.2333],
  "Rod El-Farag": [30.075, 31.24],
  "Sphinx Square": [30.057, 31.195],
  "Gameat El Dowal El Arabiya": [30.055, 31.19],
  "Ain Shams University": [30.077, 31.29],
  "Mataria Palace": [30.13, 31.31],
};
...
const zones: Record<string, Zone> = {
  "Downtown - Tahrir & Kasr El Nil": {
    ...
    radius: 0.004,
    majorStreets: ["Tahrir Square", "Kasr El Nil St", "Mohamed Mahmoud St"],
  },
  "Nasr City - Abbas El-Akkad": {
    ...
    radius: 0.006,
    majorStreets: ["Abbas El-Akkad St", "Makram Ebeid St", "El-Tayaran St"],
  },
  "Maadi - Road 9 & Road 233": {
    ...
    radius: 0.005,
    majorStreets: ["Road 9", "Road 233", "Corniche El-Maadi"],
  },
  "Heliopolis - Merghany & Al-Ahram": {
    ...
    radius: 0.005,
    majorStreets: ["Merghany St", "Al-Ahram St", "Nozha St"],
  },
  "Zamalek - 26th July & Brazil": {
    ...
    radius: 0.003,
    majorStreets: ["26th July St", "Brazil St", "El-Gezira St"],
  },
  "Giza - Pyramids Road & Faisal": {
    ...
    radius: 0.005,
    majorStreets: ["Pyramids Road", "Faisal St", "El-Nil St"],
  },
};
...
const LiveMapView: React.FC = () => {
  ...
  const selectZone = useCallback(
    (name: string) => {
      ...
      if (selectedZoneName && zoneLayersRef.current[selectedZoneName]) {
        zoneLayersRef.current[selectedZoneName].setStyle({
          fillOpacity: 0.15,
          weight: 2,
        });
      }
  ...
  async function getRealRoadRoute(
    ...
    try {
      ...
      if (j.paths && j.paths.length > 0) {
        const path: [number, number][] = j.paths[0].points.coordinates.map(
          (c: [number, number]) => [c[1], c[0]],
        );
  ...
  const applySpeed = () => {
    ...
    if (dr.animationInterval) {
      ...
      const tick = () => {
        ...
        if (path && path.length >= 2) {
          const idx = Math.min(
            path.length - 2,
            Math.floor((prog / 100) * (path.length - 1)),
          );
    ...
    setDrivers((prev) =>
      prev.map((d) =>
        d.id === dr.data.id ? { ...d, speed: Math.round(currentSpeed) } : d,
      ),
    );
  };
  ...
  const selectedZone = useMemo(
    () => (selectedZoneName ? zones[selectedZoneName] : null),
    [selectedZoneName],
  );
    ...
    <div className="app-container">
      ...
      <div className="header">
        <div className="header-left">
          <h1>
            <i className="fas fa-map-marked-alt"></i> Cairo Advanced Tracking
          </h1>
        ...
        <div className="header-right">
          ...
          <div className="system-status">
            <i className="fas fa-circle" style={{ color: "#2ecc71" }}></i> Live
            Tracking
          </div>
        </div>
      </div>
      ...
      <div className="main-content">
        ...
        <div className="left-panel">
          ...
          <div className="selection-section">
            <div className="section-title">
              <span>
                <i className="fas fa-user-tie"></i> Select Driver
              </span>
            </div>
                ...
                >
                  ...
                  <div className="driver-info">
                    ...
                    <div
                      style={{
                        fontSize: 10,
                        color: d.status === "available" ? "#2ecc71" : "#e74c3c",
                      }}
                    >
          ...
          <div className="selection-section">
            <div className="section-title">
              <span>
                <i className="fas fa-map-marked-alt"></i> Select Zone
              </span>
            </div>
          ...
          <div className="selected-info">
            {!selectedDriver && !selectedZone && (
              <>
                <h3>
                  <i className="fas fa-info-circle"></i> Selection Summary
                </h3>
                <p>
                  Select a driver and a zone to plan a route on detailed Cairo
                  maps.
                </p>
              </>
            )}
            ...
            {selectedZone && !selectedDriver && (
              <>
                ...
                <p>
                  <strong>Zone:</strong> {selectedZoneName}
                  <br />
                  <strong>Major Streets:</strong>{" "}
                  {selectedZone?.majorStreets.join(", ")}
                  <br />
              ...
              <>
                <h3>
                  <i
                    ...
                    style={{ color: "#2ecc71" }}
                  ></i>{" "}
                  Ready for Cairo Route
                </h3>
                <p>
                  <strong>Driver:</strong> {selectedDriver.name}
                  <br />
                  <strong>Destination:</strong>{" "}
                  {selectedZoneName?.split(" - ")[0]}
                  <br />
                  <strong>Vehicle:</strong> {selectedDriver.vehicle}
                  <br />
                  <span style={{ fontSize: 11, color: "#95a5a6" }}>
                    <i className="fas fa-info-circle"></i> Will follow detailed
                    Cairo roads
                  </span>
                </p>
              </>
        ...
        <div className="map-container">
          ...
          <div className="map-controls">
            ...
            <button className="map-btn" onClick={locate} title="Locate Me">
              <i className="fas fa-location-arrow"></i>
            </button>
          </div>

          {/* Status Overlay (مختصر) */}
          <div className="map-overlay status-overlay">
            <div className="status-title">
              <span>
                <i className="fas fa-tachometer-alt"></i> Journey Status
              </span>
            </div>
            <div className="status-details">
              <div className="status-item">
                ...
                <span className="status-value">
                  {selectedDriver?.name ?? "None"}
                </span>
              </div>
              <div className="status-item">
                <span className="status-label">Selected Zone:</span>
                <span className="status-value">
                  {selectedZoneName?.split(" - ")[0] ?? "None"}
                </span>
              </div>
              <div className="status-item">
                ...
                <span className="status-value">
                  {selectedDriverId
                    ? (
                        driverLayersRef.current[selectedDriverId]
                          ?.totalDistance ?? 0
                      ).toFixed(1)
                    : "0"}{" "}
                  km
            ...
            <div className="speed-control-section">
              <div className="status-title" style={{ marginBottom: 8 }}>
                <span>
                  <i className="fas fa-tachometer-alt"></i> Speed Control
                </span>
              </div>
              <div className="speed-control">
                ...
                >
                  <span className="status-label">Current Speed:</span>
                  <span className="status-value">
                    {currentSpeedSetting} km/h
                  </span>
                </div>
                <div style={{ display: "flex", gap: 8, marginBottom: 5 }}>
                  <button
                    className={`speed-btn speed-slow ${currentSpeedSetting === 20 ? "active" : ""}`}
                    onClick={() => setCurrentSpeedSetting(20)}
                  >
                    <i className="fas fa-turtle"></i> Slow
                  </button>
                  <button
                    className={`speed-btn speed-normal ${currentSpeedSetting === 40 ? "active" : ""}`}
                    onClick={() => setCurrentSpeedSetting(40)}
                  >
                    <i className="fas fa-car"></i> Normal
                  </button>
                  <button
                    className={`speed-btn speed-fast ${currentSpeedSetting === 60 ? "active" : ""}`}
                    onClick={() => setCurrentSpeedSetting(60)}
                  >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCurrentSpeedSetting(parseInt(e.target.value))
                    }
                    id="speedSlider"
                    style={{ flex: 1 }}
                  />
                ...
                <button
                  ...
                  disabled={
                    !(
                      selectedDriverId &&
                      driverLayersRef.current[selectedDriverId]?.isAnimating
                    )
                  }
                >
          ...
          <div className="map-overlay legend">
            <h4>
              <i className="fas fa-key"></i> Map Legend
            </h4>
          ...
          {/* Progress Overlay (اختياري تكملة لاحقاً) */}
          <div className="map-overlay progress-overlay">
            <div className="status-title">
              <span>
                <i className="fas fa-ruler"></i> Journey Progress
              </span>
            </div>
            <div className="progress-item">
              ...
              <span className="progress-value">
                {selectedDriverId
                  ? (
                      driverLayersRef.current[selectedDriverId]?.progress ?? 0
                    ).toFixed(1)
                  : "0"}
                %
              </span>
            ...
            <div className="progress-container">
              ...
              <div className="progress-text">
                Real Road Journey: {/* ساعة محاكاة مبسّطة */}
