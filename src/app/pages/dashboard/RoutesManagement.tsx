...
export default function RoutesManagement() {
  ...
  const toggleComparisonSelection = (
    id: string,
    e: React.MouseEvent,
  ) => {
    e.stopPropagation();
  ...
  const handleRouteAction = (
    ...
    if (action === "start") {
      notify.success(
        "Route Started",
        `${routeName} has been initiated.`,
      );
    } else if (action === "pause") {
      notify.warning(
        "Route Paused",
        `${routeName} is currently on hold.`,
      );
    } else if (action === "alert") {
      notify.critical(
        "Route Alert",
        `Immediate attention required for ${routeName}`,
        "View Details",
      );
    } else if (action === "complete") {
      notify.success(
        "Route Completed",
        `${routeName} has been finished successfully.`,
      );
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Manage and track daily collection trajectories
          </p>
        </div>
          ...
          {(currentRole === "admin" ||
            ...
            <button
              ...
              onClick={() =>
                notify.info(
                  "Create Route",
                  "The route creation wizard will open shortly.",
                )
              }
            >
                ...
                >
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          {currentRole !== "driver" && (
                            ...
                            >
                              <div
                                className={`w-5 h-5 border-2 rounded-lg flex items-center justify-center transition-colors shadow-sm ${selectedForComparison.includes(route.id) ? "bg-indigo-600 border-indigo-600 text-white" : "border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800"}`}
                              >
                          ...
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${statusStyles[route.status]}`}
                          >
                            {route.status}
                          </span>
                        </div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400 line-clamp-1">
                          {route.id} • {route.driver}
                        </p>
                      </div>
                    </div>
                      ...
                      {currentRole === "driver" ? (
                        <>
                          <button
                            ...
                            className="px-3 py-1.5 text-xs font-bold bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg flex items-center gap-1 shadow-sm"
                          >
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg flex items-center gap-1 shadow-sm"
                          >
                        ...
                        <>
                          <button
                            ...
                            className="p-2 text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-900/30 rounded-xl cursor-pointer"
                          >
                          ...
                          <button
                            ...
                            className="p-2 text-orange-600 hover:bg-orange-50 dark:text-orange-400 dark:hover:bg-orange-900/30 rounded-xl cursor-pointer"
                          >
                          ...
                          <button
                            ...
                            className="p-2 text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/30 rounded-xl cursor-pointer"
                          >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
                    <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl">
                      <p className="text-xs text-gray-500 dark:text-gray-400 font-medium mb-1 flex items-center gap-1">
                  ...
                  <div>
                    ...
                    <div className="w-full bg-gray-100 dark:bg-gray-800 h-2.5 rounded-full overflow-hidden relative">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{
                          width: `${(route.completedStops / route.stops) * 100}%`,
                        }}
                        transition={{
                          duration: 1,
                          ease: "easeOut",
