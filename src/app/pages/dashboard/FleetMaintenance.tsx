...
export default function FleetMaintenance() {
  ...
  const repairingCount = vehicles.filter(
    (v) => v.status === "In Repair",
  ).length;
      ...
      >
        <div>
          ...
          <p className="text-gray-600 mt-2 font-medium">
            Tracking vehicle health, mileage, and scheduled
            services
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          ...
          <div className="relative w-full md:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-orange-500 outline-none text-sm font-medium"
            />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {vehicles
                .filter(
                  (v) =>
                    activeTab === "all" ||
                    v.status.toLowerCase() === activeTab,
                )
                .map((vehicle, idx) => (
                  <tr
                    key={vehicle.id}
